...
*/
function loadMem(value) {
	var addr=parseInt(value, 16);
	if (isNaN(addr)) {
...
function IndirectYAddr() {
	if (addcycles) {
		...
		var a2=(a1+y)&0xffff;
		if ((a1&0xff00)!=(a2&0xff00)) extracyles++;
		return a2;
	}
	else {
...
function iff() { _KernelInterruptQueue.enqueue( new Interrput(PROGRAMCALL_IRQ, null) ); } //SYS CALL
